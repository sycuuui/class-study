...
**GET /students**
- 요청 파라미터
- `limit` (선택, 기본 50, 유효범위 1~200)
- `offset` (선택, 기본 0, 유효범위 0 이상)
- 유효범위 위반 시 400 + error.code 500 반환
...
- `offset` (선택, 기본 0, 유효범위 0 이상)
- 유효범위 위반 시 400 + error.code 500 반환
- `page.total`은 `departmentId` 필터 적용 후 총량
- 결정 사항: 존재하지 않는 `departmentId`는 200 + empty items 반환(추천안)
